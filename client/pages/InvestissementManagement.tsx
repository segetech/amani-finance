...
import {
  DollarSign, Plus, Edit, Eye, ExternalLink, TrendingUp, PieChart, Target, Users
} from 'lucide-react';
...
export default function InvestissementManagement() {
  ...
  const stats = [
    ...
    { label: 'ROI Moyen', value: '12.5%', icon: TrendingUp, bg: 'bg-purple-100' },
    { label: 'Investisseurs', value: '156', icon: Users, bg: 'bg-orange-100' },
  ];

  const sections = [
    'Opportunités d\'Investissement', 'Conseils Financiers', 'Analyses de Marché', 'Portefeuilles Recommandés'
  ];
    ...
    <DashboardLayout
      ...
      actions={
        <div className="flex gap-4">
          <button onClick={() => window.open('/investissement', '_blank')} className="flex items-center gap-2 px-4 py-2 border rounded-lg hover:bg-gray-50">
    ...
    >
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow">
          <div className="p-6 border-b"><h2 className="text-xl font-semibold">Sections Investissement</h2></div>
          <div className="divide-y">
            {sections.map((section, index) => (
              <div key={index} className="p-6 flex items-center justify-between hover:bg-gray-50">
                <div className="flex items-center gap-4">
                  <DollarSign className="w-8 h-8 text-green-600" />
                  <div><h3 className="font-medium">{section}</h3><p className="text-sm text-gray-600">Gérer le contenu de cette section</p></div>
                </div>
