    ...
    const loadPodcast = async () => {
      ...
      try {
        ...
        const mockPodcast: Partial<UnifiedContent> = {
          ...
          summary: 'Notre analyse des tendances économiques de la semaine en Afrique de l\'Ouest.',
          description: 'Un podcast hebdomadaire qui décortique les principales actualités économiques de la région.',
          status: 'published',
          category: 'economie',
          country: 'mali',
          tags: ['économie', 'analyse', 'afrique'],
          meta_title: 'Analyse économique hebdomadaire | Amani Finance',
          meta_description: 'Notre analyse des tendances économiques de la semaine en Afrique de l\'Ouest.',
          ...
          podcast_data: {
            ...
            platforms: [
              { name: 'Spotify', url: 'https://open.spotify.com/episode/123', icon: 'spotify' },
              { name: 'Apple Podcasts', url: 'https://podcasts.apple.com/episode/123', icon: 'apple' },
              { name: 'Anchor', url: 'https://anchor.fm/amani-finance/episodes/episode-1', icon: 'anchor' }
            ]
          }
        };
    ...
    >
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${
              podcast.status === 'published' ? 'bg-green-500' : 
              podcast.status === 'draft' ? 'bg-yellow-500' : 'bg-gray-500'
            }`}></div>
            <span className="text-sm text-gray-600 capitalize">
              {podcast.status === 'published' ? 'Publié' : 
               podcast.status === 'draft' ? 'Brouillon' : 'Archivé'}
            </span>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Podcast créé le {new Date(podcast.created_at || '').toLocaleDateString('fr-FR')}
                {podcast.views && (
          ...
          {podcast.podcast_data?.platforms && podcast.podcast_data.platforms.length > 0 && (
            <div className="mt-4">
